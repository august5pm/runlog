...
Google 로그인 후 러닝 세션을 기록하고, **주·월 거리 차트**와 **대시보드 날씨 요약**을 보는 웹 앱입니다.
...
- **대시보드:** 기간별 횟수·거리·운동 시간, 주·월 막대 차트(`?range=month`), 최근 기록 5건
- **날씨:** 공공데이터포털 기상청 API(초단기실황·초단기예보), 헤더 우측 스냅샷 + SVG 아이콘(키 없으면 안내 문구만)
- **반응형:** 모바일 하단 탭 · 데스크톱 상단 헤더 (`components/AppChrome.tsx`)
...
인증된 요청만 허용합니다. 자세한 쿼리·바디는 기획서 및 `app/api` 참고.
