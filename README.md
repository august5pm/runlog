...
- **인증:** Google OAuth, JWT 세션 + Prisma 어댑터 (미설정 시 랜딩에서 안내). 세션 콜백마다 User DB를 다시 읽지 않고 **JWT에 닉네임·이모지 등을 실어 두며**, 프로필 저장 시 `session.update()`로 갱신
...
- **커뮤니티:** 챌린지·자유 한 줄 글, 선택적 러닝 기록 첨부, **댓글**·**이모지 반응** (`/community`). 피드 서버 렌더는 **댓글 본문 없이** 건수만 포함하고, 카드에서 `GET .../comments`로 댓글을 불러 옴. 반응/댓글 작성 후 **전체 `router.refresh()` 없이** API 응답으로 UI 갱신
- **설정:** 닉네임·프로필 이모지(프리셋)·주/월 거리 목표 (`PATCH /api/user/profile`), **배지 모음**(조건 달성 시 이모지 배지 저장·표시, 푸시 없음)
- **날씨:** 공공데이터포털 기상청 API(초단기실황·초단기예보), 헤더 우측 스냅샷 + **PTY/SKY → 이모지**(`WeatherIcon`, 키 없으면 안내 문구만). 동일 격자·관측 슬롯에 대해 **`unstable_cache`(약 180초)** 로 외부 호출 부담 완화
- **UX:** 로그인 후 하위 경로 전환 시 `app/(main)/loading.tsx` 스켈레톤 표시
...
배지·커뮤니티 댓글 등 후속 마이그레이션이 있다면 배포 후에도 같은 명령으로 적용합니다.
